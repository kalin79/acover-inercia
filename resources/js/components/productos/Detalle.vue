        ...
        <div class="containerFluid">
            <div class="layoutGrid">
                <div class="productoImagenesContainer">
                    ...
                    class="main-slider"
                    >
                        <SplideSlide v-for="(image, index) in product.media" :key="index">
                            <div class="productoViewContainer" v-if="image.type === 'image'">
                                <img 
                                    :src="`/storage/${image.file_path}`"
                                    class="main-image"
                                />
                            </div>
                            <div class="productoViewContainer negro" v-if="image.type === 'video'">
                                <video controls>
                                    <source :src="`/storage/${image.file_path}`" type="video/mp4">
                                    Your browser does not support the video tag.
                                </video>
                            </div>
                            <div class="productoViewContainer negro" v-if="image.type === 'youtube'">
                                <iframe
                    ...
                    >
                        ...
                        >
                            <img 
                                ...
                                class="thumbnail"
                            />
                            <div v-else-if="image.type === 'video'" class="thumbnail-video">
                                <video :src="`/storage/${image.file_path}`" muted></video>
                                <div class="play-overlay">▶</div>
                            </div>
                            <div v-else-if="image.type === 'youtube'" class="thumbnail-youtube">
                                <img :src="getYoutubeThumbnail(image.youtube_url)" alt="YouTube">
                                <div class="play-overlay">▶</div>
                            </div>
                        </SplideSlide>
                    </Splide>
                ...
                <div class="productoInfoContainer">
                    ...
                    <div class="descripcionProdContainer" v-html="product.descripcion"></div>
                    <div class="btnContainer">
                        <a href="" target="_blank">
                            <img :src="icon" alt="" />
                            <span>Consultar por Whatsapp</span>
                        </a>
                        <button>Pide tu cotización</button>
                    </div>
                    ...
                    <div class="caracetisticasContainer">
                        ...
                        <div class="tablaContainer">
                            ...
                            >
                                <h4>{{feature.group}}</h4>
                                <div>
                                    <div class="itemContainer" v-if="feature.type === 'text'">
                                        <div>{{feature.name}}</div>
                                        <div>{{feature.pivot.value}}</div>
                                    </div>
                                    <div class="itemContainer itemColoresContainer" v-else-if=" feature.id===7">
                                        <!-- para los colores  -->
                                        <div
                                           v-for="(item, i) in parseValue(feature.pivot?.value)"
                                            :key="i"
                            ...
                            <div class="rowContainer">
                                <a :href="`/storage/${product.technical_document}`" target="_blank" class="btnTransparente">Descarga Ficha Técnica</a>
                            </div>
                        </div>
                    </div>
                </div>
...
const dimensionesGenerales = computed(() => {
    return props.product.features.filter(f => f.group === 'Dimensiones Generales');
});
