<template>
    <div class="bannerHome">
        <!-- {{ banners.data.banners }} -->
        <Splide v-if="banners" :options="options" class="splide-custom">
            <SplideSlide v-for="(img, index) in banners" :key="index">
                <div class="homePageHeaderContainer">
                    <div class="containerFluid">
                        <div class="gridLayout">
                            <div>
                                <h1 v-html="img.titulo"></h1>
                            </div>
                            <div>
                                <div class="alinearInferior">
                                    <p v-html="img.descripcion"></p>
                                    <div class="botonesContainer">
                                        <a href="https://wa.link/i0xeew" target="_blank" class="btnRelleno">
                                            <img :src="icon2" alt="">
                                            <span>Consultar por Whatsapp</span>
                                        </a>
                                        <a href="/" class="btnSinRelleno">
                                            Ver Catálogo
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="slideContainer">
                    <Link :href="img.link">
                      <img :src="img.poster" alt="Slide" class="imgBanner" />
                    </Link>
                </div>
            </SplideSlide>
        </Splide>
    </div>
</template>

<script setup>
import { Link } from '@inertiajs/vue3'
import { Splide, SplideSlide } from '@splidejs/vue-splide'
import '@splidejs/vue-splide/css'
const icon2 = '/images/wa2.svg';
const options = {
//   type: 'loop',
  perPage: 1,
  autoplay: true,
  interval: 5000,
  pauseOnHover: true,
  arrows: true,
  pagination: true,
  classes: {
    // Flechas
    arrows: 'splide__arrows my-arrows-home',
    arrow: 'splide__arrow my-arrow-home',
    prev: 'splide__arrow--prev my-arrow-home-prev',
    next: 'splide__arrow--next my-arrow-home-next',

    // Paginación
    pagination: 'splide__pagination my-pagination-home',
    page: 'splide__pagination__page my-page-home',
  }
}

const banners = [
    {
        poster: '/images/b1.webp',
        link: '/',
        titulo: `LOCKERS DE ALTA <br />
            DURABILIDAD <span>PARA <br />
            GIMNASIOS</span>`,
        descripcion: `Lockers diseñados para resguardar tus <br />
            pertenencias, con alta resistencia a la humedad.`,
    },
    {
        poster: '/images/2.webp',
        link: '/',
        titulo: `SISTEMA DE CUSTODIA  <br /><span>PARA INSTITUCIONES EDUCATIVAS</span>`,
        descripcion: `Lockers diseñados para garantizar seguridad, orden <br />y resistencia en entornos de alta exigencia.`,
    },
    {
        poster: '/images/3.webp',
        link: '/',
        titulo: `SISTEMA DE CUSTODIA  <br /><span>PARA INSTITUCIONES EDUCATIVAS</span>`,
        descripcion: `Lockers diseñados para garantizar seguridad, orden <br />y resistencia en entornos de alta exigencia.`,
    },
    {
        poster: '/images/4.webp',
        link: '/',
        titulo: `SOLUCIONES <br />INTEGRALES <br />
                <span>DE EMBALAJE</span>`,
        descripcion: `Soluciones de embalaje resistentes y eficientes <br />para proteger y transportar productos en entornos <br />logísticos de alta exigencia.`,
    }
]


</script>

<style>
/* Ahora usas Tailwind directamente */

.splide__arrow.my-arrow-home{
    top: 75%;
    transform: translateY(75%);
    width: 90px;
    height: 90px;
}

.splide__arrows.my-arrow-home {
    width: 70px;
    height: 70px;
    background: transparent;
    border: 1px solid white;
}

.splide__arrow.my-arrow-home svg {
    /* width: 33px;
    height: 66px;
    fill: white; */
    display: none;
}

/* Agregamos tu propio SVG como fondo */
.splide__arrow.my-arrow-home::before {
    content: '';
    display: block;
    width: 16px;
    height: 49px;
    fill: white;
    color: white;
    background: currentColor;           /* toma el color del texto */
    mask-image: url('/images/arrow-left.svg');   /* ← tu SVG */
    mask-size: contain;
    mask-repeat: no-repeat;
    mask-position: center;
    -webkit-mask-image: url('/images/arrow-left.svg');
}

.splide__arrow--next.my-arrow-home-next::before {
  mask-image: url('/images/arrow-right.svg');
  -webkit-mask-image: url('/images/arrow-right.svg');
}

.splide__pagination__page.my-page-home {
    width: 18px;
    height: 18px;
    background: transparent;
    position: relative;
    border: 1px solid white;
}

.splide__pagination__page.my-page-home:before{
    content: '';
    position: absolute;
    width: 10px;
    height: 10px;
    border-radius: 50%;
    background: #fff;
    top: 50%;
    left: 50%;
    transform: translate(-50%,-50%);
}

.splide__pagination__page.my-page-home.is-active {
    background: transparent;
}



@media screen and (min-width: 992px){
    .splide__arrow.my-arrow-home {
        width: 90px;
        height: 90px;
        top: 50%;
        transform: translateY(-50%);
    }
    .splide__arrow.my-arrow-home::before {
        width: 22px;
        height: 55px;
    }
}

@media screen and (min-width: 1400px){
    .splide__pagination__page.my-arrow-home {
        width: 114px;
        height: 114px;
    }
    .splide__arrow.my-arrow-home::before {
        width: 33px;
        height: 66px;
    }
}
</style>